...
tf_version_tuple = tuple(map(int, tensorflow.version.VERSION.split(".")))
if tf_version_tuple >= (2, 4, 0):
...
def get_writer_model():
    ...
    outputs = [
        keras.layers.Lambda(lambda x: x[:, -1:])(output) for output in decoder.outputs
    ]
