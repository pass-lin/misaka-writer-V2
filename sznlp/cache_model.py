...
class GatedAttentionUnit_cross_cache(GatedAttentionUnit_cross):
    def call(self, inputs, mask=None, a_bias=False, p_bias=None, **kwargs):
        ...
        if not K.int_shape(c)[1] or K.int_shape(kwargs.get('c_cache'))[1] != K.int_shape(c)[1] :
            c=self.kv_dense(c)
...
class Misaka_decoder_cache(Misaka_decoder):
    ...
    def apply_main_layers(self, inputs, index):
        ...
        k_cache_1=keras.layers.Input([None,self.attention_key_size],
                                     name=self_attention_1_name+'-kcache')
        self.cache_inputs.append(k_cache_1)
        v_cache_1=keras.layers.Input([None,self.intermediate_size],
                                     name=self_attention_1_name+'-vcache')
        self.cache_inputs.append(v_cache_1)
        
        cross_attention_name = 'Misaka-Dncoder-%d-GatedAttentionUnit-cross' % index
        
        c_cache_cross=keras.layers.Input([None,self.intermediate_size+self.attention_key_size],
        ...
        k_cache_2=keras.layers.Input([None,self.attention_key_size],
                                     name=self_attention_2_name+'-kcache')
        self.cache_inputs.append(k_cache_2)
        v_cache_2=keras.layers.Input([None,self.intermediate_size],
                                     name=self_attention_2_name+'-vcache')
