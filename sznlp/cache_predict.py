...
class Seq2SeqGenerate_Cache():
    ...
    def initial_cache(self,num):
        #初始化cache
        self.caches=[]
        for t in self.decoder.inputs[2:]:
            self.caches.append(np.zeros([num,0,t.shape[-1]]))
    def DcoderPredict(self,encoder_outputs,decoder_out):
        #decoder模型预测
        decoder_out=np.reshape(decoder_out,[-1,1])
        pred=self.decoder.predict([encoder_outputs,decoder_out]+self.caches,batch_size=self.batch_size)
        self.caches=pred[1:]
        return pred[0]
    ...
    def generate_sentence(self,data,#输入的独热数据
        ...
        for step in range(max_len):
            ...
            scores= self.DcoderPredict(encoder_outputs[end_token],decoder_out)  # 计算当前得分
            scores=scores[:,-1]
            ...
            output_ids = np.concatenate([output_ids, np.zeros([output_ids.shape[0],1],dtype='int')],
                                        1)  # 更新输出
    ...
    def writer(self,data,#文本数据
        ...
        try:
            for y in ys:
                result.append(self.tokenizer.decode(y).replace(self.skip_token, '\n'))
        except:
